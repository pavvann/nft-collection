...
export default function Home() {
  ...
  const publicMint = async () => {
    try {
      const signer = await getProviderOrSigner(true);
      const nftContract = new ConstructorFragment(NFT_CONTRACT_ADDRESS, abi, signer);
  ...
  const renderButton = () => {
    ...
    if (presaleStarted && !presaleEnded) {
      return (
        <div>
          ...
          <button className={styles.button} onClick={presaleMint}>Presale Mint ⚡️</button>
        </div>
      );
    }
    ...
    if (presaleStarted && presaleEnded) {
      return (
        <button className={styles.button} onClick={publicMint}>Public Mint ⚡️</button>
      );
    }
